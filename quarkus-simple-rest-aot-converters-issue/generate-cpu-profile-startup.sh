...
pushd target
java -agentpath:${PATH_TO_ASYNC_PROFILER}/lib/libasyncProfiler.so=start,event=cpu,interval=1,timeout=2,simple,file=../$JFR_FILE -XX:AOTCache=app.aot -jar code-with-quarkus-1.0.0-SNAPSHOT-runner.jar &
PID=$!
sleep 5
kill $PID
sleep 8
