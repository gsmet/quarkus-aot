...
pushd target
java -agentpath:${PATH_TO_ASYNC_PROFILER}/lib/libasyncProfiler.so=start,alloc=100,total,event=alloc,timeout=2,file=../$JFR_FILE -Xlog:aot -XX:AOTCache=quarkus-app/app.aot -jar quarkus-app/quarkus-run.jar &
PID=$!
sleep 5
kill $PID
sleep 8
